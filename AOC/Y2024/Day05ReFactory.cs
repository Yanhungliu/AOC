...
public class Day05ReFactory : AocFixture
{
    ...
    private List<List<int>> _testCases;
    private List<List<int>> _updates;

    [OneTimeSetUp]
    public void Setup()
    {
        // var sample = Input.Get<string>(); 
        
        
        var rules = sample.Split('\n', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .Where(x => x.Contains('|'))
            .Select(x => x.Split('|').Select(int.Parse).ToArray())
            .Select(parts => (Key: (parts[0], parts[1]), Value: -1))
            .ToDictionary(pair => pair.Key, pair => pair.Value);

        foreach (var kv in rules.ToList())
        {
            rules[(kv.Key.Item2, kv.Key.Item1)] = 1;
        }


        _testCases = sample.Split('\n', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .Where(x => x.Contains(','))
            .Select(x => x.Split(',').Select(int.Parse).ToList()).ToList();

        _updates = _testCases.Select(list => list.ToList()).ToList();
        ...
        foreach (var update in _updates)
        {
            update.Sort((a, b) =>
            {
                ...
                if (rules.TryGetValue((a, b), out var result))
                    return result;
                if (rules.TryGetValue((b, a), out var reverseResult))
    ...
    public void Part1()
    {
        var answer = _testCases.Select((original, index) => (original, sorted: _updates[index]))
            .Where(pair => pair.original.SequenceEqual(pair.sorted))
            .Sum(pair => pair.sorted[pair.sorted.Count / 2]);
